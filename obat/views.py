...
def update_f_obat(request, nof_obat):
    ...
    if request.method == 'POST':
        form = FormObat(request.POST, instance=fobats)
        if form.is_valid():
            form.save()
            messages.success(request, 'Data berhasil diubah')
            return redirect('obat:update_obat', nof_obat=nof_obat)
    else:
        form = FormFObat(instance=fobats)
        fobats = {
            'form': form,
            'fobats': fobats
        }
...
def delete_f_obat(request, kode_obat):
    fobats = Fungsi_Obat.objects.filter(id=kode_obat)
    fobats.delete()
    messages.success(request, "Data terhapus")
    return render(request, 'fungsi_obat/list_f_obat.html')
